import Link from 'next/link'
import React from 'react'

const Header = () => {
  return (
    <div className="header-footer sticky top-0 left-0 bg-white">
      <Link href={'/'} className="text-2xl">
        <span className="underline  text-red-500 font-bold">Break</span>
        <span className="underline fonte-medium">Bricks</span>
      </Link>
      <Link
        href={'/play'}
        className="flex justify-around gap-1 items-center border-b-4 border-red-500 shadow-inner text-lg transition px-4 p-1"
      >
        Play
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth="1.5"
          stroke="currentColor"
          className="size-4 text-red-500 transition"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"
          />
        </svg>
      </Link>
    </div>
  )
}

export default Header
